...
func (h *PhotoHandler) Upload(c *gin.Context) {
	...
	if err := db.DB.Collection("folders").FindOne(ctx, bson.M{"_id": folderId, "userId": userId}).Decode(&folder); err != nil {
...
func (h *PhotoHandler) Stream(c *gin.Context) {
	...
	var p models.Photo
	if err := db.DB.Collection("photos").FindOne(ctx, bson.M{"_id": photoId, "userId": userId}).Decode(&p); err != nil {
